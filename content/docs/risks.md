...
| State | Meaning |
|-------|-------|
| `suggested` | Generated by the threat model, awaiting review |
...
| Route | Purpose |
|-------|-------|
| `GET /threats/risks` | Risks list, filterable by state and coverage view |
| `GET /threats/risks/{id}/drawer` | Drawer partial (HTMX) |
| `POST /threats/risks/{id}/hunt` | Create hunt + transition to `hunting` |
| `POST /threats/risks/{id}/accept-residual` | Transition to `residual_accepted` |
| `POST /threats/risks/{id}/mark-active-threat` | Transition to `active_threat` |
| `POST /threats/risks/{id}/schedule-rehunt` | Re-hunt a `mitigated` risk |
| `POST /threats/paths/quick` | Model a brief's uncovered techniques into an attack path |
...
- [Threat Feed](/docs/threat-feed/) — where briefs are scored and related to risks.
