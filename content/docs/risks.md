...
A promoted rule can also trigger a re-hunt automatically without waiting for the 90-day clock. Each time an analyst verdicts an alert as TP or FP, the rule's rolling precision is updated. When precision drops below **40%** with at least 10 total verdicts, the platform immediately returns the linked risk to `hunting`. The dashboard's **Low-precision promoted rules** card surfaces rules approaching this threshold (precision < 60%, ≥10 verdicts) before the auto-rehunt fires.

---

## Stale notifications

A Temporal workflow runs every Monday at 09:00 UTC and scans for attack paths that have stalled:

| State | Stale threshold |
|-------|----------------|
| `accepted` | > 14 days since `reviewed_at` (or `created_at` if never reviewed) |
| `hunting` | > 30 days since `last_hunted_at` (or `created_at` if never hunted) |

For each stale path the platform sends a notification with a direct link to the Risk Ops Board. No state transitions are forced — the notification is advisory. If your notification channel is not configured the workflow completes silently.

The workflow uses an overlap policy of **skip**, so a backed-up run does not queue duplicate notifications for the following week.
